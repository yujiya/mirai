...
public class IpUtils {
    ...
    //根据IP和mask确定需要被扫的List
    public static List<String> parseIpMaskRange(String ip,String mask){
        List<String> list=new ArrayList<String>();
        if ("32".equals(mask)) { //如果mask是32的话  集合中只需要加入自身IP
            list.add(ip);
        }else{
            String startIp=getBeginIpStr(ip, mask);//计算IP段起始
            String endIp=getEndIpStr(ip, mask);//计算IP段终止
            if (!"31".equals(mask)) {//如果不是31端口   重新计算起始IP和终止IP
    ...
    /**
     * 根据 ip/掩码计算IP段的起始IP �?IP�?218.240.38.69/30
     *
     * @param ip
    ...
    /**
     * 根据 ip/掩码计算IP段的终止IP �?IP�?218.240.38.69/30
     *
     * @param ip
     *            给定的IP，如218.240.38.69
     * @param maskBit
    ...
    public static Long getEndIpLong(String ip, String maskBit)
    {
      ...
      /**
     * 根据子网掩码转换为掩码位 255.255.255.252转换为掩码位 30
     *
     * @param netmarks
     * @return
     */
    ...
    /*
     * 存储锁有的掩码位及对应的掩码key:掩码�?value:掩码（x.x.x.x�?     */
